...
export const AppsHeader: React.FC<AppsHeaderProps> = ({
  ...
  return (
    <header className="flex-shrink-0 sticky top-0 z-40 bg-white dark:bg-[#2C1D29] text-slate-800 dark:text-white border-b border-slate-200/80 dark:border-slate-800 transition-all shadow-xs dark:shadow-none select-none">
      <div className="w-full px-3 sm:px-6 lg:px-8 h-11 sm:h-12 flex items-center justify-between gap-4">
            ...
            {activeSubTitle ? (
              <>
                ...
                <button
                  ...
                  className="p-1.5 -ml-1 rounded-lg text-slate-500 hover:text-[#F15A24] dark:text-slate-400 dark:hover:text-orange-400 hover:bg-orange-50/80 dark:hover:bg-orange-950/40 transition-colors cursor-pointer shrink-0 flex items-center justify-center"
                  title="Về Trang chủ AVG One"
                >
                  <Home className="w-4 h-4 sm:w-4.5 sm:h-4.5 stroke-[2.3]" />
                </button>

                {/* 2. Chữ phân hệ: CHUYỂN ĐỔI TRỰC TIẾP CHO TO HƠN, CĂN LỀ TRÁI */}
                <button
                  onClick={onBack}
                  className="group text-sm sm:text-base font-black text-[#F15A24] dark:text-orange-400 uppercase tracking-wide cursor-pointer flex items-center hover:text-orange-600 dark:hover:text-orange-300 transition-colors select-none shrink-0 py-0.5"
