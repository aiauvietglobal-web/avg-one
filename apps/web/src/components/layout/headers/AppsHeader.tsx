...
export const AppsHeader: React.FC<AppsHeaderProps> = ({
  ...
  return (
    <header className="flex-shrink-0 sticky top-0 z-40 bg-white dark:bg-[#2C1D29] text-slate-800 dark:text-white border-b border-slate-200/80 dark:border-slate-800 transition-all shadow-xs dark:shadow-none select-none">
      <div className="w-full px-3 sm:px-6 lg:px-8 h-12 sm:h-13 flex items-center justify-between gap-4">
            ...
            {activeSubTitle ? (
              <>
                ...
                <button
                  ...
                  className="h-8 sm:h-8.5 w-8 sm:w-8.5 -ml-1 rounded-lg text-slate-500 hover:text-[#F15A24] dark:text-slate-400 dark:hover:text-orange-400 hover:bg-orange-50/80 dark:hover:bg-orange-950/40 transition-colors cursor-pointer shrink-0 flex items-center justify-center leading-none"
                  title="Về Trang chủ AVG One"
                >
                  <Home className="w-4.5 h-4.5 stroke-[2.2] shrink-0" />
                </button>

                {/* 2. Chữ phân hệ: CHUYỂN ĐỔI TRỰC TIẾP CHO TO HƠN, CĂN LỀ TRÁI */}
                <button
                  onClick={onBack}
                  className="group h-8 sm:h-8.5 px-1 text-sm sm:text-base font-black text-[#F15A24] dark:text-orange-400 uppercase tracking-wide cursor-pointer flex items-center leading-none hover:text-orange-600 dark:hover:text-orange-300 transition-colors select-none shrink-0"
              ...
              <img
                ...
                className="h-6 sm:h-7 object-contain cursor-pointer pointer-events-auto shrink-0"
                onClick={() => onSelectModule('home')}
