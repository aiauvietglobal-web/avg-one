...
export const AppsHeader: React.FC<AppsHeaderProps> = ({
  ...
  return (
    <header className="flex-shrink-0 sticky top-0 z-40 bg-white dark:bg-[#2C1D29] text-slate-800 dark:text-white border-b border-slate-200/80 dark:border-slate-800 transition-all shadow-xs dark:shadow-none select-none">
      <div className="w-full px-3 sm:px-6 lg:px-8 h-11 sm:h-12 flex items-center justify-between gap-4 sm:gap-6">
        {/* Left: Chuyển đổi trực tiếp / Phân hệ con (căn lề trái, cân đối, hiện mũi tên khi hover/ấn) hoặc Logo AVG One */}
        <div className="flex items-center gap-2.5 sm:gap-3 select-none shrink-0 whitespace-nowrap">
          {activeSubTitle ? (
            <button
              onClick={onBack}
              className="group text-sm sm:text-base lg:text-[17px] font-black text-[#F15A24] dark:text-orange-400 uppercase tracking-tight cursor-pointer flex items-center hover:text-orange-600 dark:hover:text-orange-300 transition-colors select-none shrink-0 py-0.5"
              title="Quay lại danh mục ứng dụng"
            >
              <span className="inline-flex items-center overflow-hidden transition-all duration-200 ease-out w-0 opacity-0 -translate-x-1 group-hover:w-4.5 group-hover:opacity-100 group-hover:translate-x-0 group-hover:mr-1 group-active:w-4.5 group-active:opacity-100 group-active:translate-x-0 group-active:mr-1 group-focus-visible:w-4.5 group-focus-visible:opacity-100 group-focus-visible:translate-x-0 group-focus-visible:mr-1 shrink-0">
                <ChevronLeft className="w-4.5 h-4.5 stroke-[2.8] text-[#F15A24] dark:text-orange-400 shrink-0" />
              </span>
